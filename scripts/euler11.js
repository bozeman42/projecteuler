...
"01 70 54 71 83 51 54 69 16 92 33 48 61 43 52 01 89 19 67 48"];
  // Turn grid into array of arrays of integers
    console.log(grid);
  grid = grid.map(function(lines){
    return lines.split(" ").map(function(numstrings){
  ...
  for (var i = 0; i < grid.length; i++) {
    for (var j = 0; j < grid[i].length; j++) {
      if (j < grid[i].length -4){
        temp = multiplyFourGridNums(grid,i,j,0,1);
        maxProduct = (temp > maxProduct)?temp:maxProduct;
        if (i < grid.length - 4) {
          temp = multiplyFourGridNums(grid,i,j,1,1);
          maxProduct = (temp > maxProduct)?temp:maxProduct;
        }
      }
      if (i < grid.length - 4) {
        temp = multiplyFourGridNums(grid,i,j,1,0);
        maxProduct = (temp > maxProduct)?temp:maxProduct;
        if (j > 2) {
          temp = multiplyFourGridNums(grid,i,j,1,-1);
          maxProduct = (temp > maxProduct)?temp:maxProduct;
