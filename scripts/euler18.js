var main = function() {
    ...
    for (var i = triangle.length - 2; i >= 0; i--) {
        triangle[i] = triangle[i].map(function (el,ind) {
            return (el+(Math.max(triangle[i+1][ind],triangle[i+1][ind+1])));
        });
        console.log(triangle[i]);
    }
    $("#answer").html(triangle[0][0]);
    $("#calc-time").html(performance.now()-t0);
};
